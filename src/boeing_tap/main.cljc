(ns boeing_tap.main
  "Kotodama WASM entrypoint for the Boeing_tap clean-room actor (L4) — Clojure port.
  ...
  facts (`boeing_tap.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "boeing_tap")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "boeingta_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "boeingta_par"
    :fields [:caseId :role :name]          :required [:role :name]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents" :id-prefix "boeingta_doc"
    :fields [:caseId :type :contentRef]     :required [:type]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents" :id-prefix "boeingta_inc"
    :fields [:type :jurisdiction :occurredAt] :required [:type :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "boeingta_wat"
    :fields [:name :program :entries]      :required [:name :program]
    :coerce {:entries :int}          :refs {}}
   {:entity "Sanction"  :plural "sanctions" :id-prefix "boeingta_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"boeing_tap.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "boeing_tap-compat" :tier tier :entities entities} 200])
...
(defn start! [] :boeing_tap-compat/ready)
